...
if __name__ == "__main__":
    ...
    trainset = CelebA("data", "train", download=False, transform=transforms.Compose([transforms.ToTensor(), transforms.Resize((64,64))]))
    trainloader = DataLoader(trainset, 64, shuffle=True, num_workers=4, persistent_workers=True, prefetch_factor=4)

    validset = CelebA("data", "valid", download=False, transform=transforms.Compose([transforms.ToTensor(), transforms.Resize((64,64))]))
    ...
    if load_prev:
        epoch, model, optimizer, history = load(checkpoint_path, model, optimizer, load_history=True)
        epoch += 1
    else:
        epoch = 0
        history = {
